...
{
  /*
   * We need a way to make sure our TSS destructors run at thread exit, even if
   * the thread exits via native calls instead of our own pg_thrd_exit() or
   * trampoline function.  So we register one real native 'hook' destructor
   * that will then call all the destructors in our own destructor table.
   *
   * On Windows, we use FlsAlloc(), not TlsAlloc(), because that supports
   * destructors.  Unforunately they have the wrong calling convention, or we
   * could simply use them directly instead of doing all this extra work.
   */
#ifdef PG_THREADS_WIN32
...
pg_tss_create(pg_tss_t *tss_id, pg_tss_dtor_t destructor)
{
  ...
  static pg_once_flag destructor_cleanup_once;
	...
	/*
	 * Make sure our destructor hook is registered with the operating system in
	 * this process. This happens only once in the whole process.  Making sure
	 * it will run actually in each thread happens in
...
#ifdef PG_THREADS_NEED_DESTRUCTOR_TABLE
  /*
   * We have to search the destructor table linearly, but deleting IDs
   * is probably very rare so that's OK.
   */
  pg_rwlock_wrlock(&dtor_table_lock);  
  for (size_t i = 0; i < dtor_table_count; ++i)
    {
